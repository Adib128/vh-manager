...
export class FuelsService {
  ...
  async findByVehicle(id: string): Promise<Fuel[]> {
    let fuel;
    try{
      fuel =  await this.fuelModel.find({'vehicle' : id}).exec();
    }catch(error){
      throw new NotFoundException(`Fuel record with the vehicle ID ${id} is not found`);
    }
    return fuel ; 
  }

  async findByDriver(id: string): Promise<Fuel[]> {
    let fuel;
    try{
      fuel =  await this.fuelModel.find({'vehicle' : id}).exec();
    }catch(error){
